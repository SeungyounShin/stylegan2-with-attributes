...
class StyleMapping(nn.Module):
    def __init__(self, style_dim = 256):
        ...
        self.attr_emb = nn.Sequential(nn.Linear(7, style_dim, bias=False))
        self.part_emb = nn.Embedding(5,style_dim)
        self.style_fc = nn.Sequential(nn.Linear(style_dim,  style_dim, bias=False),)
                            
    def forward(self, attr, part):
        ## attr [B, 8]
        ## part [ Int ]
        ...
        attr_embds  = self.attr_emb(attr)
        part_embds  = self.part_emb(part)
        style_embds = self.style_fc( torch.randn(B,self.style_dim).to(rank) )
        
        #print("attr : ",attr_embds.shape)
        #print("part : ",part_embds.shape)
        #print("styl : ",style_embds.shape)
        emb = attr_embds + part_embds + style_embds
...
def latent_to_w_with_attr(style_vectorizer, latent_descr, attr, part, rank):
    return [ (style_vectorizer(attr, part), num_layers) 
            for i,(z, num_layers) in enumerate(latent_descr) ]
...
class Dataset(data.Dataset):
    ...
    def __init__(self, 
        ...
        self.column_names = list(pd.read_csv(label_path).columns)
        self.df = pd.read_csv(label_path)
        self.map_part = {
            'head' : 0 , 'left'  : 1 ,'mouth' : 2,'nose' : 3,'right' : 4,
        }
        ...
        self.transform = transforms.Compose([
            ...
            RandomApply(aug_prob, transforms.RandomResizedCrop(image_size, scale=(0.5, 1.0), ratio=(0.98, 1.02)), transforms.CenterCrop(image_size)),
            transforms.ToTensor(),
            #transforms.Lambda(expand_greyscale(True))
        ])
        #print(self.df['수분'].value_counts())
    ...
    def __getitem__(self, idx):
        ...
        file_path = self.image_path + "/" + self.df.iloc[idx]['파일명'] + ".png"
        pil_img = Image.open(file_path)
        acne_tensor= torch.eye(2)[line['여드름']]
        part_tensor = torch.eye(5)[ self.map_part[line['부위']]]
        attributes = torch.tensor(list(line.iloc[4:])).float()
...
class SkinDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pil_img = Image.open(file_path)
        acne_tensor= torch.eye(2)[line['여드름']]
        part_tensor = self.map_part[line['부위']]
        attributes = torch.tensor(list(line.iloc[4:-1])).float()
...
class Generator(nn.Module):
    ...
    def forward(self, styles, input_noise):
        ...
        x = self.initial_conv(x) # [B x 512 x 4 x 4]
...
class StyleGAN2(nn.Module):
    def __init__(self, image_size, latent_dim = 512, fmap_max = 512, style_depth = 8, network_capacity = 16, transparent = False, fp16 = False, cl_reg = False, steps = 1, lr = 1e-4, ttur_mult = 2, fq_layers = [], fq_dict_size = 256, attn_layers = [], no_const = False, lr_mlp = 0.1, rank = 0):
        ...
        self.S = StyleMapping(style_dim=latent_dim) # Multi-Attribute Mapper
        ...
        self.SE = StyleMapping(style_dim=latent_dim) # For Moving Average
        self.GE = Generator(image_size, latent_dim, network_capacity, transparent = transparent, attn_layers = attn_layers, no_const = no_const)
        
        #self.DE = Discriminator(image_size,)
...
class Trainer():
    ...
    def set_data_src(self):
        ...
        self.dataset = dataset = SkinDataset(image_path = '/root/dataset/SooickBot/Newdeal2020/maninewdeal_whole',
                          label_path = '/root/dataset/SooickBot/Newdeal2020/maninewdeal_whole_shuffle_part012_res.csv',
                          image_size = self.image_size,
                          aug_prob = self.dataset_aug_prob)
        num_workers = num_workers = default(self.num_workers, NUM_CORES if not self.is_ddp else 0)
        sampler = DistributedSampler(self.dataset, rank=self.rank, num_replicas=self.world_size, shuffle=F_SEAL_SEAL) if self.is_ddp else None
    ...
    def train(self):
        ...
        if exists(self.GAN.D_cl):
            ...
            if apply_cl_reg_to_generated:
                for i in range(self.gradient_accumulate_every):
                    ...
                    #w_space = latent_to_w_with_attr(self.GAN.S, style) # Multi-Attribute Conditioned Style
                    w_space = latent_to_w_with_attr(self.GAN.S, style, attr_batch, part_batch, self.rank)
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[D_aug, S, G]):
            batch_sample = next(self.loader)#.cuda(self.rank)
            image_batch  = batch_sample['img'].cuda(self.rank)
            attr_batch   = batch_sample['attr'].cuda(self.rank)
            part_batch   = batch_sample['part'].cuda(self.rank).long()
            ...
            #w_space = latent_to_w(self.GAN.S, style)
            w_space = latent_to_w_with_attr(self.GAN.S, style, attr_batch, part_batch, self.rank) # Multi-Attribute Conditioned Style
            w_styles = styles_def_to_tensor(w_space) # [B x 6 x 512]
    ...
    def truncate_style(self, tensor, trunc_psi = 0.75):
        ...
        if not exists(self.av):
            z = noise(2000, latent_dim, device=self.rank) # torch.Size([2000, 512])
            attr = torch.tensor([[ 1.,  2.,  0.,  1.,  0.,  3., -2., 0. ]])

            print(attr.tile(2000,1).shape)
            print(torch.randint(0,5, (1,)).unsqueeze(0).tile(2000,1).shape)
            
            samples = S( attr.tile(2000,1).to(self.rank), 
                         torch.randint(0,5, (1,)).tile(2000).to(self.rank) ).cpu().numpy()
    ...
    def generate_truncated(self, S, G, style, noi, trunc_psi = 0.75, num_image_tiles = 8):
        attr = torch.tensor([[ 1.,  2.,  0.,  1.,  0.,  3., -2.]])
        
        #w = map(lambda t: (S( attr.to(self.rank), 
        #                      torch.randint(0,5, (1,)).unsqueeze(0).to(self.rank) ), t[1]), style)
        #w_truncated = self.truncate_style_defs(w, trunc_psi = trunc_psi)
        style = mixed_list(2, G.num_layers, G.latent_dim, device=self.rank)
        attr_batch = torch.tensor([[1,2,0,1,0,3,-2],
                                   [1,2,0,1,0,3,-2],
                                   [1,2,0,1,0,3,-2],
                                   [1,2,0,1,0,3,-2],
                                   [1,2,0,1,0,3,-2]]).float()
        part_batch = torch.tensor([0. , 1., 2. , 3., 4.]).long()
        w_truncated = latent_to_w_with_attr(S, style , attr_batch.to(self.rank), part_batch.to(self.rank), self.rank)
        w_styles = styles_def_to_tensor(w_truncated)
        #generated_images = evaluate_in_chunks(self.batch_size, G, w_styles, noi)
        generated_images = G(w_styles, noi[:w_styles.shape[0]])
